 ...
 *
 * This is the base repository class for review entities.
 */
class Reviews_Entity_Repository_Base_Review extends EntityRepository
{
    /**
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $listHelper = new Reviews_Util_ListEntries(ServiceUtil::getManager());
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('review', 'workflowState');
                $templateParameters['scoreItems'] = $listHelper->getEntries('review', 'score');
            }
    
            // initialise Imagine preset instances
            $imageHelper = new Reviews_Util_Image(ServiceUtil::getManager());
    
            $objectType = 'review';
            $templateParameters[$objectType . 'ThumbPresetCoverUpload'] = $imageHelper->getPreset($objectType, 'coverUpload', $context, $args);
            if (in_array($args['action'], array('display', 'view'))) {
                // use separate preset for images in related items
                $templateParameters['relationThumbPreset'] = $imageHelper->getCustomPreset('', '', 'Reviews_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters = array();
        $parameters['catIdList'] = ModUtil::apiFunc('Reviews', 'category', 'retrieveCategoriesFromRequest', array('ot' => 'review', 'source' => 'GET'));
        ...
        $parameters['score'] = isset($this->controllerArguments['score']) ? $this->controllerArguments['score'] : FormUtil::getPassedValue('score', '', 'GET');
        $parameters['zlanguage'] = isset($this->controllerArguments['zlanguage']) ? $this->controllerArguments['zlanguage'] : FormUtil::getPassedValue('zlanguage', '', 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('Reviews_Entity_Review', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('Reviews_Entity_Review', 'tbl')
     ...
     *
     * @return array|Reviews_Entity_Review retrieved data array or Reviews_Entity_Review instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved Reviews_Entity_Review instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     *
     * @return Reviews_Entity_Review retrieved instance of Reviews_Entity_Review
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('Reviews', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'review', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if ($currentType == 'admin' && $currentModule == 'Reviews') {
            return $qb;
        }
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.text LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.zlanguage LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.reviewer LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.email = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.score = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.url = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.url_title LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.cover LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.coverUpload = \'' . $fragment . '\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.workflowState = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numReviews';
        if ($useJoins === true) {
     ...
     * @param int    $excludeId  Id of reviews to exclude (optional).
     *
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        ...
        $showOnlyOwnEntries = (int) FormUtil::getPassedValue('own', ModUtil::getVar('Reviews', 'showOnlyOwnEntries', 0), 'GETPOST');
