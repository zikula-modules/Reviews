 ...
 */
class Reviews_Block_Base_ItemList extends Zikula_Controller_AbstractBlock
{
    /**
    ...
    {
        SecurityUtil::registerPermissionSchema('Reviews:ItemListBlock:', 'Block title::');
    ...
    public function info()
    {
        ...
        return array('module'          => 'Reviews',
                     'text_type'       => $this->__('Reviews list view'),
                     'text_type_long'  => $this->__('Display list of Reviews objects.'),
                     'allow_multiple'  => true,
                     'form_content'    => false,
                     'form_refresh'    => false,
                     'show_preview'    => true,
    ...
    public function display($blockinfo)
    {
        ...
        if (!SecurityUtil::checkPermission('Reviews:ItemListBlock:', "$blockinfo[title]::", ACCESS_OVERVIEW)) {
            return false;
        }
        ...
        if (!isset($vars['catIds'])) {
            $primaryRegistry = ModUtil::apiFunc('Reviews', 'category', 'getPrimaryProperty', array('ot' => $vars['objectType']));
        ...
        if (in_array($objectType, $this->categorisableObjectTypes)) {
            if (is_array($vars['catIds']) && count($vars['catIds']) > 0) {
                $qb = ModUtil::apiFunc('Reviews', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => $objectType, 'catids' => $vars['catIds']));
            }
        }
        ...
        if (empty($blockinfo['title'])) {
            $blockinfo['title'] = $this->__('Reviews items');
        }
    ...
    protected function getSortParam($vars, $repository)
    {
        ...
        if ($vars['sorting'] == 'newest') {
            $idFields = ModUtil::apiFunc('Reviews', 'selection', 'getIdFields', array('ot' => $vars['objectType']));
            if (count($idFields) == 1) {
    ...
    public function update($blockinfo)
    {
        ...
        $vars['objectType'] = $this->request->request->filter('objecttype', 'review', FILTER_SANITIZE_STRING);
        ...
        $vars['catIds'] = array($primaryRegistry => array());
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            $vars['catIds'] = ModUtil::apiFunc('Reviews', 'category', 'retrieveCategoriesFromRequest', array('ot' => $vars['objectType']));
        }
