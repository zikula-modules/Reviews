 ...
 */
class Reviews_Util_Base_Workflow extends Zikula_AbstractBase
{
    /**
    ...
    public function getActionsForObject($entity)
    {
        ...
        $listHelper = new Reviews_Util_ListEntries($this->serviceManager);
    ...
    public function collectAmountOfModerationItems()
    {
        ...
        if (SecurityUtil::checkPermission($modname . ':' . ucwords($objectType) . ':', '::', ACCESS_ADD)) {
            ...
            if ($amount > 0) {
                $amounts[] = array(
                    'aggregateType' => 'reviewsApproval',
                    'description' => $this->__('Reviews pending approval'),
                    'amount' => $amount,
                    'objectType' => $objectType,
                    'state' => $state,
                    'message' => $this->_fn('One review is waiting for approval.', '%s reviews are waiting for approval.', $amount, array($amount))
                );
            }
        }
